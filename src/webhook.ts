...
export class WebhookAPI {
    ...
    constructor(config: WebhookConfig = {}) {
        ...
        this.app.all("/", (req: any, res) => {
            ...
            if (!this.verifySignature(req.rawBody, signature)) return res.status(401).send("Unauthorized");
    ...
    verifySignature(body: string, signature: string) {
        ...
        if (!this.config.secret) throw new Error("Webhook secret not provided.");
